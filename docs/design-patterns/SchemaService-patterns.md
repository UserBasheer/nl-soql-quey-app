...
| `isCustom()` is already on the `DescribeSObjectResult` the loop holds, so the correct matching form costs no extra describe call. | The predicate now needs describe context, not just a name — `isSystemObject()` takes `(apiName, isCustomObject)` instead of a bare string. It is still directly unit-testable with synthetic inputs (marked `@TestVisible` for exactly this reason — see `SchemaServiceTest`), but any caller now has to supply the `isCustom()` flag itself rather than relying on a name-only check. |
