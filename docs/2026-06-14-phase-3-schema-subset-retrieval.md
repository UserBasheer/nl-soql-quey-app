...
> **Note (2026-09-10):** A defect was found (via scratch-org validation, before Phase 3
> could deploy) in the noise-suffix exclusion described below: it only matched the
> `'__'+suffix` form, which exists solely for **custom**-object siblings
> (`MyObject__Share`). Standard-object siblings — `AccountShare`, `CaseHistory`,
> `OpportunityFeed`, `AccountChangeEvent`, etc., the majority of real-org noise — were
> never excluded. This was fixed on `feature/2026-09-10-fix-system-object-filter`; the
> sections below marked **[pre-fix]** describe the original, incomplete behavior. See
> `docs/2026-09-10-schema-service-noise-filter-fix.md` for the fix and current behavior,
> and `force-app/main/default/classes/SchemaService.cls` (`isSystemObject`) for the
...
2. **Permission + noise filtering (Apex)** — `listObjects()` iterates `Schema.getGlobalDescribe().values()`. For each `SObjectType`, it keeps the object only if:
   ...
   - it is not a custom setting (`isCustomSetting()`), not deprecated/hidden (`isDeprecatedAndHidden()`), and its API name does not end in `__Share`, `__History`, `__Feed`, `__ChangeEvent`, or `__Tag` (case-insensitive, via the new `isSystemObject()` helper). **[pre-fix — `isSystemObject()` originally checked only the `'__'+suffix` form unconditionally; it now also checks the bare-suffix form for standard objects. See `docs/2026-09-10-schema-service-noise-filter-fix.md`.]**
...
| `listObjectsExcludesKnownSystemNoiseSuffixes`                        | **new**  | No returned `apiName` ends in `__share`, `__history`, `__feed`, `__changeevent`, or `__tag` (case-insensitive). Spot-checks that `Account` is retained while `AccountShare`/`AccountHistory`/`AccountFeed`/`AccountChangeEvent` (if present in `getGlobalDescribe()`) are excluded. **[This is the assertion that later caught the pre-fix bug — the `AccountShare`-style spot-check failed against a scratch org until the 2026-09-10 fix; see `docs/2026-09-10-schema-service-noise-filter-fix.md`. Test count below (9 methods) reflects this phase only; 3 more were added by the fix, for 12 total.]** |
...
- **Limitations:**
  - The system-suffix exclusion (`__Share`, `__History`, `__Feed`, `__ChangeEvent`, `__Tag`) is a fixed heuristic. A (rare) legitimately named custom object ending in one of these suffixes — e.g. a custom `__Tag` business object — would be hidden from the picker even if accessible. This is an accepted trade-off per the design pattern doc, since these suffixes are reserved system patterns in practice. **[The exclusion logic itself was buggy as originally shipped — it only matched the `'__'+suffix` form, missing standard-object siblings entirely. Fixed 2026-09-10; this trade-off statement is accurate for the current, fixed behavior. See `docs/2026-09-10-schema-service-noise-filter-fix.md`.]**
...
| 2026-09-10 | Annotated in place (not rewritten) to flag that the noise-suffix exclusion described in this doc originally matched only the `'__'+suffix` form and missed standard-object noise siblings (`AccountShare`, `CaseHistory`, etc.) — a real defect caught by scratch-org validation and fixed on `feature/2026-09-10-fix-system-object-filter`. See `docs/2026-09-10-schema-service-noise-filter-fix.md` for the fix and current behavior. No functional content in this doc was changed, only marked where it describes pre-fix behavior. |
